...
function fetchAndDisplayNotifications($conn) {
    ...
    $query = "SELECT * FROM notification WHERE n_user_id = '{$_SESSION['current_user']}' AND n_type IN ('greeting','response')  ORDER BY n_time DESC";
...
function getBusinessUserInfo($conn, $userId) {
    $query = "SELECT cp_username AS username, cp_profile_img_url AS profile_img FROM customer_profile WHERE c_id = '$userId'";
    $result = mysqli_query($conn, $query);
    if ($result && mysqli_num_rows($result) > 0) {
