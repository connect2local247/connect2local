...
";
    ...
    if(isset($_SESSION['business_id'])){
        $get_verification_status = "SELECT two_step_status FROM business_verification WHERE b_id = '{$_SESSION['business_id']}'";
    ...
    <?php if($two_step_enabled == 1){
            $update = "UPDATE business_verification SET b_verification_code = '$verification_code' WHERE b_id = '{$_SESSION['business_id']}'";
    ...
    ?>
        ...
        <script>
            path = "/user/businessman/login/form/email_verification.php";
        </script>
    <?php }else{ ?>
        <script>
    ...
    <?php 
        ...
        <form action="/user/businessman/login/code/login-validation.php" method="post" class="d-flex flex-column align-items-center justify-content-center p-2" style="height:90vh;width: 100%;">
            <fieldset class="p-3 border rounded col-lg-7 col-md-10 col-12 text-white" id="register-form">
                ...
                <div class="mt-3">
                    ...
                    <input type="email" name="email" class="form-control border border-dark border-2 py-2" id="email" placeholder="Email Address" value="<?php if(isset($_SESSION['email'])) echo $_SESSION['email'] ?>" required>
                </div>

                <div class="mt-3">
                    <label for="password" class="form-label ps-1">Password</label>
                    <div class="mt-1 position-relative">
                        <input type="password" name="password" id="password" class="form-control p-2 border-2 border-dark" placeholder="Password" required>
        <i id="togglePassword" class="fas fa-eye text-secondary position-absolute top-0 end-0 py-2 fs-5 mt-1 me-4"></i>
                ...
                <div class="mt-3 d-flex align-items-center justify-content-between px-2">
                    <a href="/user/businessman/register/form/business_register.php" class="nav-link" style="font-size:15px;color:skyblue">Create an Account</a>
                    <a href="/user/businessman/login/form/verification-email.php" class="nav-link" style="font-size:15px;color:skyblue">Forgot Password</a>
                 </div>
